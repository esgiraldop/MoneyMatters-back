...
export class SeedersService {
  ...
  async insertCategories(): Promise<void> {
    ...
    const existingNames = existingCategories.map((category: any) => category.name);

    const newCategories = categories.filter((category) => !existingNames.includes(category));
    ...
    const values = newCategories.map((category, index) => `(${index + 1}, '${category}')`).join(', ');
